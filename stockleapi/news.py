...
def get_items(query, cache=True, mock=False):
    """
    Access the IBM news API, search for the input keyword. For news item,
    pass it to the sentimentanalysis API (AlchemyAPI).
    Returns a list of news item URLS with a sentiment score.

    Input: a string containing the name of a company.
    Output: JSON containing a list of items (title, URL, sentiment)
    """
    if cache:
        with open('cache/news_{0}.json'.format(query)) as f:
        ...
        try:
            ...
            log.error('no news items found for {0}'.format(query))
            print query
            print results
        return items
